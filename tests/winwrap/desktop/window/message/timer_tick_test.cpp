...
struct TickingWindow : winwrap::user::Window<TickingWindow> {
...
struct QuietWindow : winwrap::user::Window<QuietWindow> {
...
TEST_CASE("a raw SetTimer callback keeps its ticks away from on_timer") {
    ...
    winwrap::user::message::send(window->hwnd(), WM_TIMER, 3,
...
struct ClosingTimerWindow : winwrap::user::Window<ClosingTimerWindow> {
