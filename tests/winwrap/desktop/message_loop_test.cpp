...
struct PumpWindow : winwrap::user::Window<PumpWindow> {
    static constexpr const wchar_t* class_name = L"WinwrapPumpTestWindow";
...
struct ClosableWindow : winwrap::user::Window<ClosableWindow> {
    static constexpr const wchar_t* class_name = L"WinwrapClosableTestWindow";
    void on_destroy() { winwrap::user::message_loop::quit(); }
};
}  // namespace

TEST_CASE("run() returns the posted exit code when WM_QUIT is already queued") {
    winwrap::user::message_loop::quit(42);
    CHECK(winwrap::user::message_loop::run() == 42);
}
...
TEST_CASE("run() dispatches a posted message to the window before it quits") {
    ...
    REQUIRE(winwrap::user::message::post(window->hwnd(), WM_COMMAND, 7, 0));
    winwrap::user::message_loop::quit(0);
...
TEST_CASE("run() drains all queued messages in order, then quits") {
    ...
    REQUIRE(winwrap::user::message::post(hwnd, WM_COMMAND, 1, 0));
    REQUIRE(winwrap::user::message::post(hwnd, WM_COMMAND, 2, 0));
    REQUIRE(winwrap::user::message::post(hwnd, WM_COMMAND, 3, 0));
    winwrap::user::message_loop::quit(0);
    CHECK(winwrap::user::message_loop::run() == 0);
...
TEST_CASE("closing the window exits run() via on_destroy -> quit") {
    ...
    REQUIRE(window.has_value());
    REQUIRE(winwrap::user::message::post(window->hwnd(), WM_CLOSE, 0,
