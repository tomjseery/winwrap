...
- **M2 / result-bearing hooks and setup.** `WW_CASE` discards returns, including
  `on_create() == -1`; `on_created()` cannot return setup failure. The dispatcher
...
- **M9 / focused headers depend on an umbrella macro.** `mixins.hpp` defines
  `WW_CASE`; it is not a pure aggregator. Focused hook-header instantiation fails
  without it. Window and Control include the whole mixin dependency set.
  **Resolve:** standalone public-header instantiation tests and self-contained
  implementation; retain/drop optional aggregators based on measured usability
  and compile cost, not an include-cleaner warning alone.
- **MSVC empty-base size claims.** Prior measurement recorded 24-byte Window versus
  16 with `__declspec(empty_bases)` on MessageDispatcher. The source's zero-size
  implication is not portable. **Resolve:** soften the claim or apply a justified
  compiler-specific optimization with current measurements. Low priority.
