...
[[nodiscard]] std::error_code configuration_error(CONFIGRET result) {
    return win32_error(::CM_MapCrToWin32Err(result, ERROR_GEN_FAILURE));
}
...
std::expected<std::size_t, Device::ControlError> control(HANDLE handle, DWORD code,
    ...
    if (input.size() > maximum || output.size() > maximum)
        return std::unexpected(Device::ControlError{.code = win32_error(ERROR_INVALID_PARAMETER)});
    ...
    if (returned > output.size())
        return std::unexpected(Device::ControlError{.code = win32_error(ERROR_INVALID_DATA),
...
std::expected<std::vector<std::wstring>, std::error_code> paths(
    ...
    if (characters.empty())
        return std::unexpected(win32_error(ERROR_INVALID_DATA));
...
std::expected<std::vector<std::wstring>, std::error_code> Device::paths(const GUID& interface_id) {
    ...
    return std::unexpected(win32_error(ERROR_RETRY));
}
