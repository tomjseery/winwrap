...
/// list, no free typing). It composes notification::SelectionChange, so
/// `on_selection_changed(index)` fires when the user picks a different item (the reflected
/// `CBN_SELCHANGE`). Populate it with `add_item`, and read or set the current pick with
