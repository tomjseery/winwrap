...
/// to the control: CommandReflection sends it, the control's mixins (e.g. Click)
...
/// message handled; otherwise pass. (See the mixins in
/// <winwrap/desktop/window/notification/command/>.)
...
inline std::optional<LRESULT> handle_command(UINT msg, WPARAM wparam, WORD code,
...
std::optional<LRESULT> handle_command(UINT msg, WPARAM wparam, WORD code, const Handler& handler,
                                      Fetch fetch) {
    if (msg == wm_command_reflect && HIWORD(wparam) == code) {
        if (handler)
            handler(fetch());
