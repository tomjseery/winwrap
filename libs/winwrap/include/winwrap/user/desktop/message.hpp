...
namespace winwrap::user::message {

/// Sends `msg` to `hwnd` and waits until its window procedure returns (SendMessageW).
...
inline LRESULT send(HWND hwnd, UINT msg, WPARAM wparam = 0, LPARAM lparam = 0) {
    return SendMessageW(hwnd, msg, wparam, lparam);
}

/// Queues `msg` for `hwnd` and returns immediately (PostMessageW); the window receives it
...
///         instead.
[[nodiscard]] inline std::expected<void, std::error_code> post(HWND hwnd, UINT msg,
                                                               WPARAM wparam = 0,
                                                               LPARAM lparam = 0) {
    if (!hwnd)
