...
[[nodiscard]] std::expected<HMODULE, std::error_code> loaded(const std::filesystem::path& name);
...
[[nodiscard]] std::expected<std::filesystem::path, std::error_code> path(HMODULE module);
...
/// Its operations delegate to the winwrap::user::module functions, so raw-handle code can use either.
class Module final {
public:
    ...
    /// The running executable (module::current()).
    [[nodiscard]] static Module current() noexcept { return Module{module::current()}; }

    /// An already-loaded module by name or path (module::loaded()).
    [[nodiscard]] static std::expected<Module, std::error_code> loaded(
        const std::filesystem::path& name) {
        return module::loaded(name).transform([](HMODULE handle) { return Module{handle}; });
    }

    /// The file this module was loaded from (module::path()).
    [[nodiscard]] std::expected<std::filesystem::path, std::error_code> path() const {
        return module::path(handle_);
    }
