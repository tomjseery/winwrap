...
#include "winwrap/shared/device/control_code.hpp"
...
public:
    ...
    [[nodiscard]] static std::expected<std::vector<std::wstring>, std::error_code> paths(
        const winwrap::shared::device::Interface& interface_id) {
    ...
    [[nodiscard]] std::expected<std::size_t, ControlError> control(
        winwrap::shared::device::ControlCode code, std::span<const std::byte> input,
